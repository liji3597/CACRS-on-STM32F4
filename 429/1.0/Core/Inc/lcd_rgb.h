...
// 6.  只开单层显示，16位色模式下，  LTDC时钟可以设置为33M
// 7.  只开单层显示，RGB888模式下，  LTDC时钟最大只能设置为30M（不同的面板会有所差异，需要根据实际情况调整）
// 8.  只开单层显示，ARGB8888模式下，LTDC时钟最大只能设置为22M（不同的面板会有所差异，需要根据实际情况调整）
// 9.  开双层显示，RGB565和ARGB1555模式下，LTDC时钟最大只能设置为24M（不同的面板会有所差异，需要根据实际情况调整）
// 10. 开双层显示，RGB565和ARGB8888模式下，LTDC时钟最大只能设置为14M（不同的面板会有所差异，需要根据实际情况调整）
// 11. 开双层显示，RGB888和ARGB1555模式下，LTDC时钟最大只能设置为18M（不同的面板会有所差异，需要根据实际情况调整）
// 12. 开双层显示，RGB888和ARGB8888模式下，LTDC时钟最大只能设置为12M（不同的面板会有所差异，需要根据实际情况调整）
// 13. 综上，不建议FK429M2核心板开启双层，因为过低的LTDC时钟会造成屏幕闪烁或者无法正常工作！
...
// 8.  只开单层显示，16位色模式下，  LTDC时钟可以设置为33M
// 9.  只开单层显示，RGB888模式下，  LTDC时钟最大只能设置为30M（不同的面板会有所差异，需要根据实际情况调整）
// 10. 只开单层显示，ARGB8888模式下，LTDC时钟最大只能设置为22M（不同的面板会有所差异，需要根据实际情况调整）
// 11. 开双层显示，RGB565和ARGB1555模式下，LTDC时钟最大只能设置为24M（不同的面板会有所差异，需要根据实际情况调整）
// 12. 开双层显示，RGB565和ARGB8888模式下，LTDC时钟最大只能设置为14M（不同的面板会有所差异，需要根据实际情况调整）
// 13. 开双层显示，RGB888和ARGB1555模式下，LTDC时钟最大只能设置为18M（不同的面板会有所差异，需要根据实际情况调整）
// 14. 开双层显示，RGB888和ARGB8888模式下，LTDC时钟最大只能设置为12M（不同的面板会有所差异，需要根据实际情况调整）
// 15. 综上，不建议FK429M2核心板开启双层，因为过低的LTDC时钟会造成屏幕闪烁或者无法正常工作！
